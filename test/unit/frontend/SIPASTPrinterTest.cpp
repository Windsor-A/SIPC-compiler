...
TEST_CASE("ASTPrinterTest: conditional expression printers", "[ASTNodePrint]") {
  ...
  std::vector<std::string> expected{"(x==y)", "(x!=y)", "(x>y)", "(x<y)", "(x>=y)", "(x<=y)", "(xandy)", "(xory)", "(not x)"};
...
TEST_CASE("ASTPrinterTest: for test", "[ASTNodePrint]") {
  std::stringstream stream;
  stream<<R"(
    fun() {
      var x,y,z;
  ...
  std::vector<std::string> expected{"var x, y, z;", "for (x : y .. z by 1) x++;", "for (x : y) x++;", "for (x : y .. z by 2) x--;", "return 0;"};
