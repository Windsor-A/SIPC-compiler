...
TEST_CASE("Check Assignable: subscript", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y; x = [3 of y]; x[0] = 1; return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_NOTHROW(CheckAssignable::check(ast.get()));
}

TEST_CASE("Check Assignable: pointer to subscript", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y; x = [3 of y]; *x[0] = 1; return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_NOTHROW(CheckAssignable::check(ast.get()));
...
TEST_CASE("Check Assignable: subscript of subscript", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y, z; x = [3 of y]; z = [x]; z[0][1] = 2; return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_NOTHROW(CheckAssignable::check(ast.get()));
}

TEST_CASE("Check Assignable: for loop", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y, z; for(x : 1 .. y by z){} return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_NOTHROW(CheckAssignable::check(ast.get()));
}

TEST_CASE("Check Assignable: foreach loop", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y, z; x = [3 of y]; for(x[0] : x){} return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_NOTHROW(CheckAssignable::check(ast.get()));
}
...
TEST_CASE("Check Assignable: int literal in for loop", "[Symbol]") {
  std::stringstream stream;
  stream << R"(recordlhs(p) { var x, y; for(1:x..y){} return 0; })";
  auto ast = ASTHelper::build_ast(stream);
  REQUIRE_THROWS_MATCHES(CheckAssignable::check(ast.get()), SemanticError,
        ContainsWhat("1 not an l-value"));
}

TEST_CASE("Check Assignable: non assignable foreach loop ", "[Symbol]") {
  std::stringstream stream;
  stream << R"(arraylhs() { var x, y, z; x = [3 of y]; for(z + 1 : x){} return 0; })";
  auto ast = ASTHelper::build_ast(stream);
REQUIRE_THROWS_MATCHES(CheckAssignable::check(ast.get()), SemanticError,
        ContainsWhat("(z+1) not an l-value"));
}
