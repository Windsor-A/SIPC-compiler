...
void PrettyPrinter::endVisit(ASTForStmt *element) {
  ...
  if (element->getIncrement() != nullptr) {
    inc = visitResults.back();
    visitResults.pop_back();
  }
  std::string upper = visitResults.back();
  visitResults.pop_back();
  std::string lower = visitResults.back();
  visitResults.pop_back();
  std::string var = visitResults.back();
  visitResults.pop_back();
  indentLevel--;

  std::string forString = indent() + "for (" + var + " : " + lower + " .. " + upper + " by " + inc + ") \n" + body;
...
void PrettyPrinter::endVisit(ASTForEachStmt *element) {
  std::string body = visitResults.back();
  visitResults.pop_back();
  std::string iterable = visitResults.back();
  visitResults.pop_back();
  std::string var = visitResults.back();
  visitResults.pop_back();
  indentLevel--;

  std::string forEachString = indent() + "for (" + var + " : " + iterable + ") \n" + body;
...
void PrettyPrinter::endVisit(ASTLengthExpr *element) {
  ...
  visitResults.push_back("(#" + arr + ")");
}

void PrettyPrinter::endVisit(ASTSubscriptExpr *element) {
  std::string i = visitResults.back();
  visitResults.pop_back();
  std::string x = visitResults.back();
  visitResults.pop_back();

  visitResults.push_back("(" + x + "[" + i + "])");
}

void PrettyPrinter::endVisit(ASTTernaryExpr *element) {
  std::string el = visitResults.back();
  visitResults.pop_back();
  std::string th = visitResults.back();
  visitResults.pop_back();
  std::string co = visitResults.back();
  visitResults.pop_back();

  visitResults.push_back("(" + co + " ? " + th + " : " + el + ")");
}
...
void PrettyPrinter::endVisit(ASTIncrementStmt *element) {
  ...
  visitResults.push_back(indent() + x + "++;");
}
...
void PrettyPrinter::endVisit(ASTNegationExpr *element) {
  ...
  visitResults.push_back("(-" + x + ")");
}

void PrettyPrinter::endVisit(ASTBooleanNotExpr *element) {
  std::string x = visitResults.back();
  visitResults.pop_back();

  visitResults.push_back("(not " + x + ")");
}

void PrettyPrinter::endVisit(ASTArrayLiteralExpr *element) {
  std::string inner = joinWithDelim(visitResults, ", ", element->getElems().size(), 1);
  visitResults.push_back("[" + inner + "]");
}

void PrettyPrinter::endVisit(ASTArrayComprehensionExpr *element) {
  std::string inner = joinWithDelim(visitResults, " of ", 2, 1);
  visitResults.push_back("[" + inner + "]");
}
