...
// A counter to create shared counters
int ctrNum = 0;
...
llvm::Function *getFunction(std::string Name) {
   ...
   * initialization in Function::codegen().
   */
  if (Name == "main") {
...
llvm::Value *ASTBinaryExpr::codegen() {
  ...
  if (getOp() == "+") {
    ...
    auto *val = Builder.CreateMul(L, R, "_andtmp");
    auto *c1 = Builder.CreateIsNotNull(val, "_nntmp");
    return Builder.CreateIntCast(c1, IntegerType::getInt64Ty(TheContext),
                                 false, "andtmp");
  } else if (getOp() == "or") {
    auto *val = Builder.CreateAdd(L, R, "_ortmp");
    auto *c1 = Builder.CreateIsNotNull(val, "_nntmp");
    return Builder.CreateIntCast(c1, IntegerType::getInt64Ty(TheContext),
                                 false, "ortmp");
  }
...
// SIPC additions begin here
// Don't forget to add to new binary expression codegen

//TODO: IMPLEMENT THESE FUNCTIONS

//Expressions
llvm::Value *ASTArrayComprehensionExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;

  Value *noElems = getNum()->codegen();
  Value* noElemsp1 = Builder.CreateAdd(noElems, ConstantInt::get(Type::getInt64Ty(TheContext), 1));
  std::vector<Value *> twoArg;
  twoArg.push_back(noElemsp1);
  twoArg.push_back(ConstantInt::get(Type::getInt64Ty(TheContext), 8));
  auto *allocInst = Builder.CreateCall(callocFun, twoArg);
  Value *arp = Builder.CreatePointerCast(
      allocInst, Type::getInt64PtrTy(TheContext));
  Value *szp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, ConstantInt::get(Type::getInt64Ty(TheContext), 0));
  auto *storeStmt = Builder.CreateStore(noElems, szp);

  Value *ctrp = Builder.CreateAlloca(IntegerType::getInt64Ty(TheContext));
  Builder.CreateStore(ConstantInt::get(Type::getInt64Ty(TheContext), 0), ctrp);
  llvm::Function *TheFunction = Builder.GetInsertBlock()->getParent();
  labelNum++;
  BasicBlock *HeaderBB = BasicBlock::Create(
      TheContext, "header" + std::to_string(labelNum), TheFunction);
  BasicBlock *BodyBB =
      BasicBlock::Create(TheContext, "body" + std::to_string(labelNum));
  BasicBlock *ExitBB =
      BasicBlock::Create(TheContext, "exit" + std::to_string(labelNum)); // Add an explicit branch from the current BB to the header
  Builder.CreateBr(HeaderBB);
  ...
  // Emit loop header
  {
    ...
    Value *iterv = Builder.CreateLoad(Type::getInt64Ty(TheContext), ctrp);
    Value *cmp = Builder.CreateICmpSLT(iterv, noElems);
  ...
  // Emit loop body
  {
    ...
    Value *iterv = Builder.CreateLoad(Type::getInt64Ty(TheContext), ctrp);
    Value *iterv2 = Builder.CreateAdd(iterv, ConstantInt::get(Type::getInt64Ty(TheContext), 1));

    Value *bd = getVal()->codegen();
    Value *ep = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, iterv2);
    Builder.CreateStore(bd, ep);


    Builder.CreateStore(iterv2, ctrp);
    Builder.CreateBr(HeaderBB);
  }
  ...
  return Builder.CreatePtrToInt(arp, Type::getInt64Ty(TheContext));
}

llvm::Value *ASTArrayLiteralExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;

  size_t noElems = getElems().size();
  std::vector<llvm::Value *> vals;
  for (ASTExpr *elem : getElems()) {
    vals.push_back(elem->codegen());
  }


  std::vector<Value *> twoArg;
  twoArg.push_back(ConstantInt::get(Type::getInt64Ty(TheContext), noElems+1));
  twoArg.push_back(ConstantInt::get(Type::getInt64Ty(TheContext), 8));
  auto *allocInst = Builder.CreateCall(callocFun, twoArg);
  Value *arp = Builder.CreatePointerCast(
      allocInst, Type::getInt64PtrTy(TheContext));
  Value *szp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, ConstantInt::get(Type::getInt64Ty(TheContext), 0));
  auto *storeStmt = Builder.CreateStore(ConstantInt::get(Type::getInt64Ty(TheContext), noElems), szp);
  for (size_t i = 0; i < noElems; i++) {
    Value *elp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, ConstantInt::get(Type::getInt64Ty(TheContext), i+1));
    auto *storeStmt = Builder.CreateStore(vals[i], elp);
  }
  return Builder.CreatePtrToInt(arp, Type::getInt64Ty(TheContext));
}
...
llvm::Value *ASTBooleanExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  return ConstantInt::get(IntegerType::getInt64Ty(TheContext), getValue());
}

llvm::Value *ASTBooleanNotExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  Value *v = getBool()->codegen();
  Value *rt = Builder.CreateIsNull(v);
  return Builder.CreateIntCast(rt, IntegerType::getInt64Ty(TheContext), false);
}

llvm::Value *ASTLengthExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;

  Value *arv = getArr()->codegen();
  Value *arp = Builder.CreateIntToPtr(arv, Type::getInt64PtrTy(TheContext));
  Value *c0 = ConstantInt::get(IntegerType::getInt64Ty(TheContext), 0);
  Value *szp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, c0);
  return Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), szp);
}

llvm::Value *ASTSubscriptExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  ...
  Value *arv = getArr()->codegen();
  Value *arp = Builder.CreateIntToPtr(arv, Type::getInt64PtrTy(TheContext));
  Value *gind = getIndex()->codegen();
  Value *c0 = ConstantInt::get(IntegerType::getInt64Ty(TheContext), 0);
  Value *c1 = ConstantInt::get(IntegerType::getInt64Ty(TheContext), 1);
  Value *ind = Builder.CreateAdd(gind, c1);
  Value *szp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, c0);
  Value *szv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), szp);
  ...
  Value *obl = Builder.CreateICmpSLT(gind, ConstantInt::get(IntegerType::getInt64Ty(TheContext), 0));
  Value *obr = Builder.CreateICmpSGE(gind, szv);
  Value *ob = Builder.CreateLogicalOr(obl, obr);
  llvm::Function *TheFunction = Builder.GetInsertBlock()->getParent();
  labelNum++; // create shared labels for these BBs
  BasicBlock *ThenBB = BasicBlock::Create(
      TheContext, "then" + std::to_string(labelNum), TheFunction);
  ...
  Builder.CreateCondBr(ob, ThenBB, ElseBB);

  // Emit then block.
  {
    Builder.SetInsertPoint(ThenBB);

    std::vector<Value *> ArgsV(1, gind);
  ...
  Value *elp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, ind);
  if (isLvalue) {
    return elp;
  }
  return Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), elp);
}

llvm::Value *ASTNegationExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  Value *v = getVar()->codegen();
  return Builder.CreateSub(ConstantInt::get(IntegerType::getInt64Ty(TheContext), 0), v);
}

llvm::Value *ASTTernaryExpr::codegen() {
  LOG_S(1) << "Generating code for " << *this;

  Value *CondV = getVar()->codegen();
  if (CondV == nullptr) {
    throw InternalError(
        "failed to generate bitcode for the condition of the ternary expr");
  }
  ...
  Builder.CreateCondBr(CondV, ThenBB, ElseBB);
  Builder.SetInsertPoint(ThenBB);
  Value *fv = getFirst()->codegen();
  Builder.CreateBr(MergeBB);
  TheFunction->getBasicBlockList().push_back(ElseBB);
  Builder.SetInsertPoint(ElseBB);
  Value *sv = getSecond()->codegen();
  Builder.CreateBr(MergeBB);
  TheFunction->getBasicBlockList().push_back(MergeBB);
  Builder.SetInsertPoint(MergeBB);
  return Builder.CreateSelect(CondV, fv, sv);
}
...
llvm::Value *ASTIncrementStmt::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  lValueGen = true;
  Value *p = getBody()->codegen();
  lValueGen = false;
  Value *v = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), p);
  Value *a = Builder.CreateAdd(v, ConstantInt::get(IntegerType::getInt64Ty(TheContext), 1));
  return Builder.CreateStore(a, p);
}
...
llvm::Value *ASTForStmt::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  lValueGen = true;
  Value *x = getVar()->codegen();
  lValueGen = false;
  Value *l = getStart()->codegen();
  Value *u = getEnd()->codegen();
  Value *b = getIncrement() ? getIncrement()->codegen() : ConstantInt::get(IntegerType::getInt64Ty(TheContext), 1);
  Builder.CreateStore(l, x);
  ...
  // Emit loop header
  {
    ...
    Value *xv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), x);
    Value *CondV = Builder.CreateICmpSLT(xv, u);
  ...
  // Emit loop body
  {
    ...
    Value *BodyV = getStatement()->codegen();
    Value *iv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), x);
    Value *iiv = Builder.CreateAdd(iv, b);
    Builder.CreateStore(iiv, x);
...
llvm::Value *ASTForEachStmt::codegen() {
  LOG_S(1) << "Generating code for " << *this;
  Value *i = Builder.CreateAlloca(Type::getInt64Ty(TheContext), ConstantInt::get(Type::getInt64Ty(TheContext), 0));
  lValueGen = true;
  Value *x = getElement()->codegen();
  lValueGen = false;
  Value *y = getIterable()->codegen();
  Value *arp = Builder.CreateIntToPtr(y, Type::getInt64PtrTy(TheContext));
  Value *c0 = ConstantInt::get(IntegerType::getInt64Ty(TheContext), 0);
  Value *szp = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, c0);
  Value *szv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), szp);
  ...
  // Emit loop header
  {
    ...
    Value *iv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), i);
    Value *CondV = Builder.CreateICmpSLT(iv, szv);
  ...
  // Emit loop body
  {
    ...
    Value *iv = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), i);
    Value *iiv = Builder.CreateAdd(iv, ConstantInt::get(IntegerType::getInt64Ty(TheContext), 1));
    Value *ne = Builder.CreateGEP(arp->getType()->getPointerElementType(), arp, iiv);
    Value *cp = Builder.CreateLoad(IntegerType::getInt64Ty(TheContext), ne);
    // cp = Builder.CreateIntToPtr(cp, IntegerType::getInt64PtrTy(TheContext));
    Builder.CreateStore(cp, x);
    Value *BodyV = getBody()->codegen();
    Builder.CreateStore(iiv, i);
